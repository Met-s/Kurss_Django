...
Задание: Логирование
1. Консоль
    DEBUG : (время, уровень, сообщение) 'formatDebug'
    WARNING : (время, уровень, сообщение,
        путь к источнику сообщения "pathname") 'formatWarning'
    ERROR,CRITICAL : (время, уровень, сообщение, путь к источнику сообщения,
                        стек ошибки "exc_info") 'formatErrorCritical'
   Сюда должны попадать сообщения из логера "django".
   Фильтр : DEBUG = True, 'require_debug_true'
---------------
2. Файл general.log
    INFO : (время, уровень, модуль "module", сообщение) 'format2'
    Сюда должны попадать сообщения из логера "django".
    Фильтр : DEBUG = False, 'require_debug_false'
---------------
3. Файл errors.log
    ERROR, CRITICAL : 'formatErrorCritical'
    Сюда должны попадатть сообщенния из логеров
        django.request, django.server, django.template, django.db.backends
---------------
4. Файл security.log
    INFO : 'format2'
    Сюда должны попадать сообщения из логера django.security
---------------
5. email
    ERROR : 'formatWarning'
    Сюда должны попадатть сообщенния из логеров django.request, django.server
    Фильтр : DEBUG = False, 'require_debug_false'
---------------------------------------
1. Написал фильтры:
2. форматы (чтобы выводилось только время "без даты" 'datefmt': "%H:%M:%S")
3. хендлеры
4. логеры
---------------------------------------
